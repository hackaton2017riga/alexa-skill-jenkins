...
function whoBrokeTheBuild(intent, session, callback){
    var req = http.get("http://bpukdkak:DevOpsRig01@52.56.148.0/jenkins/job/ExampleWorkspace/job/ExampleProject/view/Java_Reference_Application/api/json?pretty=true&tree=jobs[name,lastBuild[number,timestamp,result,changeSet[items[author[fullName]]]]]"
    , function(res) {
...
console.log("2");
        res.on('data', function(chunk) {
            jenkinsJson += chunk;
            console.log("jenkinsJson=" + jenkinsJson);
        });
